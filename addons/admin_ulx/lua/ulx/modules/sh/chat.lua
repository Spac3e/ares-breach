...
function ulx.psay( calling_ply, target_ply, message )
	if IsValid(calling_ply) then
		...
		if !calling_ply:IsAdmin() and target_ply:GTeam() != TEAM_SPEC then
			calling_ply:AresNotify("Вы можете отправлять сообщение только наблюдателям!")
			return
		end
	end
	...
	target_ply:AresNotify(sendercolor, "[PM From: ",sendername,"] ", color_white, message)
	if IsValid(calling_ply) then
		calling_ply:AresNotify(Color(0,255,255), "[PM To: ","dont_translate:"..target_ply:Name(),"] ", color_white, message)
	end
end
...
if SERVER then
	hook.Add("PlayerSay", "ULX_MUTE_COCKERS", function(ply, msg)
		if ply:GetNWBool("ulx_muted") then
			ply:AresNotify("l:you_are_muted")
			return ""
		end
	end)
end
...
function ulx.asay( calling_ply, message )
	...
	if calling_ply:GetPData("banned_report_folife") then calling_ply:AresNotify("Вам запрещен доступ к репорту.") return end

	if !calling_ply:IsAdmin() then
		if !calling_ply.ulx_asay_cd then
			calling_ply.ulx_asay_cd = 0
		end
		if CurTime() < calling_ply.ulx_asay_cd then calling_ply:AresNotify("l:ulx_asay_cd") return end
	...
	if !admins then
		calling_ply:AresNotify("l:no_admins")
		return
	end
	...
	for _, admin in pairs(player.GetHumans()) do
		if admin:IsAdmin() or admin == calling_ply then
			if calling_ply:IsSuperAdmin() then
				admin:AresNotify(Color(0,255,255), "[Developer] ", color_white, "(", "dont_translate:"..calling_ply:Name(), ") ", "dont_translate:"..message)
			elseif calling_ply:GetUserGroup() == "spectator" then
				admin:AresNotify(Color(165,55,0), "[Warden] ", color_white, "(", "dont_translate:"..calling_ply:Name(), ") ", "dont_translate:"..message)
			elseif calling_ply:IsAdmin() then
				admin:AresNotify(Color(0,0,255), "[Admin] ", color_white, "(", "dont_translate:"..calling_ply:Name(), ") ", "dont_translate:"..message)
			else
				admin:AresNotify(Color(255,0,0), "[Report] ", color_white, "(", "dont_translate:"..calling_ply:Name(), ") ", "dont_translate:"..message)
