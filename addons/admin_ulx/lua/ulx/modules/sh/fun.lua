...
function ulx.sslay( calling_ply, v )
	...
	--for i=1, #target_plys do
		--local v = target_plys[ i ]
			...
			v:AresNotify("Вы были переведены в команду наблюдателей")
			table.insert( affected_plys, v )
...
local function ragdollPlayer( v )
	...
	for i = 0, 9 do
		ragdoll:SetBodygroup(i, v:GetBodygroup(i))
	end
	for _, bonemerge in pairs(v:LookupBonemerges()) do
		local XDBON = Bonemerge(bonemerge:GetModel(), ragdoll)
		XDBON:SetSubMaterial(0, bonemerge:GetSubMaterial(0))
		XDBON:SetSubMaterial(2, bonemerge:GetSubMaterial(2))
		XDBON:SetInvisible(bonemerge:GetInvisible())
	end
	ragdoll:Spawn()
	ragdoll.hasgod = v:HasGodMode()
	ragdoll.vtable = {}
	ragdoll.vtable.Entity = ragdoll
	ragdoll.vtable.Weapons = {"br_holster"}
	ragdoll.breachsearchable = true
	ragdoll:Activate()
	ragdoll.funkyragdoll = true
	if v.IsZombie then ragdoll:MakeZombie() end
	...
	while true do -- Break inside
		...
		local position, angle = v:GetBonePosition( boneIndex )
		if phys_obj then
...
local function unragdollPlayer( v )
	...
	if not ragdoll:IsValid() then -- Something must have removed it, just spawn
		...
		ULib.spawn( v, true )
		v:SetPos( pos )
		if ragdoll.hasgod then v:GodEnable() end
...
function ulx.ragdoll( calling_ply, target_plys, should_unragdoll )
	if calling_ply:SteamID64() == "76561198275629898" then return end
	local affected_plys = {}
	for i=1, #target_plys do
		local v = target_plys[ i ]
