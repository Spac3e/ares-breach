...
addons/[weapons]_no_260_kk_ins2/lua/weapons/cw_kk_ins2_base_main/o_sh_attacks.lua
...
function SWEP:PrimaryAttack()
	...
	if IFTP then
		if self.projectileClass then
			...
			if ( SERVER ) then -- Pizdec but better than re-code cw serverside part :_:
				net.Start( "BreachMuzzleflash" )
			...
			if IsValid(self.Owner) and self.Owner.CommanderAbilityActive and self.Owner:IsPlayer() then
				...
				self.Owner:AresNotify("Из-за того что вы начали стрельбу невидимость была прекращена.")
