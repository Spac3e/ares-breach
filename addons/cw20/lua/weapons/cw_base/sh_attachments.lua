...
function SWEP:ValidateAttachments()
    ...
    for name, status in pairs(self.ActiveAttachments) do
        ...
        for id, attachmentTable in pairs(self.Attachments) do
            for i = 1, #attachmentTable.atts do
                local attach_name = attachmentTable.atts[i]
		...
		if wep:_attach(category, pos) then
			if CustomizableWeaponry.playSoundsOnInteract then
				if CurTime() > wep.AttachSoundDelay then
					...
					if not ply.DoNotPlayInteract then
