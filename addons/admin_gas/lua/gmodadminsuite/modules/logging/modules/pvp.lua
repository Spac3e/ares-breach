...
PVP_COMBAT_MODULE:Setup(function()
	...
	local function EntityTakeDamage(victim, dmginfo)
		if not victim then return end
		if (not victim:IsPlayer() or victim:IsBot() or victim:HasGodMode() or not victim:Alive() or dmginfo:GetDamage() <= 0) then return end
		...
		if (not IsValid(attacker) or attacker:IsWorld() or not attacker:IsPlayer()) then
			local prop
		...
		elseif (IsValid(inflictor) and inflictor:IsVehicle()) then
			local driver = inflictor:GetDriver()
			if (IsValid(driver)) then
				local pvp_event = GAS.Logging.PvP:GetEvent(driver, victim)
				pvp_event:AddDamage("pvp_vehicle_damage", victim, driver, inflictor, dmginfo, GAS.Logging:FormatPlayer(victim), GAS.Logging:FormatVehicle(inflictor, false), GAS.Logging:FormatPlayer(driver))
			else
				if (GAS.Logging.PvP.PlayerEvents[account_id]) then
					for _,event in ipairs(GAS.Logging.PvP.PlayerEvents[account_id]) do
						pvp_event:AddDamage("pvp_driverless_vehicle_damage", victim, nil, nil, dmginfo, GAS.Logging:FormatPlayer(victim), GAS.Logging:FormatVehicle(inflictor, false))
					end
				end
			end
	...
	GAS.Hooking:FeedbackHook("EntityTakeDamage", "logging:PvP:TrackDamage", EntityTakeDamage)

	GAS.Hooking:InferiorHook("DoPlayerDeath", "logging:PvP:TrackDeathDmg", function(victim, attacker, dmginfo)
		if (dmginfo:GetDamage() <= 0) then return end
		EntityTakeDamage(victim, dmginfo)
	end)
	...
	GAS:hook("PlayerDeath", "logging:PvP:TrackDeath", function(victim, inflictor, _attacker)
		...
		if (not attacker:IsWorld() and attacker:IsPlayer()) then
