...
addons/[admin]_gmodadminsuite/lua/openpermissions/sh.lua
...
function OpenPermissions:GetPermission(ply, permission_id, is_operator)
	...
	if (ply:GTeam()) then
		local ply_team_registry = OpenPermissions.PermissionsRegistry[OpenPermissions.ACCESS_GROUP.TEAM .. " " .. OpenPermissions:GetTeamIdentifier(ply:GTeam())]
		...
		if (DarkRP and RPExtraTeams[ply:GTeam()]) then
			local ply_category_name = RPExtraTeams[ply:GTeam()].category
			local ply_category
